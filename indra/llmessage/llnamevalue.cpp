...
{
	std::string buffer;
	buffer = fmt::format(FMT_STRING("{:s} {:s} {:s} {:s} "), mName, mStringType, mStringClass, mStringSendto);
	buffer.append(printData());
...
std::string LLNameValue::printData() const
{
	...
	case NVT_ASSET:
		...
		break;
	case NVT_F32:
		buffer = fmt::format(FMT_STRING("{:f}"), *mNameValueReference.f32);
		break;
	case NVT_S32:
		buffer = fmt::format(FMT_STRING("{:d}"), *mNameValueReference.s32);
		break;
	case NVT_U32:
	  	buffer = fmt::format(FMT_STRING("{:d}"), *mNameValueReference.u32);
		break;
	case NVT_U64:
		{
		...
		break;
	case NVT_VEC3:
	  	buffer = fmt::format(FMT_STRING("{:f}, {:f}, {:f}"), mNameValueReference.vec3->mV[VX], mNameValueReference.vec3->mV[VY], mNameValueReference.vec3->mV[VZ]);
		break;
	default:
