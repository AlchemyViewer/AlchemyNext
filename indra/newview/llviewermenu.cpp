...
void handle_claim_public_land(void*)
{
	...
	std::string buffer;
	buffer = fmt::format(FMT_STRING("{:f}"), west_south.mV[VX]);
	msg->nextBlock("ParamList");
	msg->addString("Parameter", buffer);
	buffer = fmt::format(FMT_STRING("{:f}"), west_south.mV[VY]);
	msg->nextBlock("ParamList");
	msg->addString("Parameter", buffer);
	buffer = fmt::format(FMT_STRING("{:f}"), east_north.mV[VX]);
	msg->nextBlock("ParamList");
	msg->addString("Parameter", buffer);
	buffer = fmt::format(FMT_STRING("{:f}"), east_north.mV[VY]);
	msg->nextBlock("ParamList");
	msg->addString("Parameter", buffer);
	gAgent.sendReliableMessage();
}
