...
void LLPanelScriptLimitsRegionMemory::setRegionDetails(LLSD content)
{
	...
	args_parcels["[PARCELS]"] = fmt::to_string(number_parcels);
...
void LLPanelScriptLimitsRegionMemory::setRegionSummary(LLSD content)
{
	...
	if((mParcelMemoryUsed >= 0) && (mParcelMemoryMax >= 0))
	{
		...
		args_parcel_memory["[COUNT]"] = fmt::to_string(mParcelMemoryUsed);
		...
		if (0 < mParcelMemoryMax)
		{
			...
			args_parcel_memory["[MAX]"] = fmt::to_string(mParcelMemoryMax);
			args_parcel_memory["[AVAILABLE]"] = fmt::to_string(parcel_memory_available);
	...
	if((mParcelURLsUsed >= 0) && (mParcelURLsMax >= 0))
	{
		...
		args_parcel_urls["[COUNT]"] = fmt::to_string(mParcelURLsUsed);
		args_parcel_urls["[MAX]"] = fmt::to_string(mParcelURLsMax);
		args_parcel_urls["[AVAILABLE]"] = fmt::to_string(parcel_urls_available);
