...
BOOL LLPolyMeshSharedData::loadMesh( const std::string& fileName )
{
        ...
        if(fileName.empty())
        {
                ALOG_CRITICAL("Filename is Empty!");
                return FALSE;
        }
        LLFILE* fp = LLFile::fopen(fileName, "rb");                     /*Flawfinder: ignore*/
        if (!fp)
        {
                ALOG_CRITICAL("can't open: {}", fileName);
                return FALSE;
        }
        ...
        if ( strncmp(header, HEADER_BINARY, strlen(HEADER_BINARY)) == 0 )       /*Flawfinder: ignore*/
        {
                ...
                if (numRead != 1)
                {
                        ALOG_CRITICAL("can't read HasWeights flag from {}", fileName);
                        return FALSE;
                }
                if (!isLOD())
                {
                ...
                if (numRead != 1)
                {
                        ALOG_CRITICAL("can't read HasDetailTexCoords flag from {}", fileName);
                        return FALSE;
                }
                ...
                if (numRead != 3)
                {
                        ALOG_CRITICAL("can't read Position from {}", fileName);
                        return FALSE;
                }
                ...
                if (numRead != 3)
                {
                        ALOG_CRITICAL("can't read RotationAngles from {}", fileName);
                        return FALSE;
                }
                ...
                if (numRead != 1)
                {
                        ALOG_CRITICAL("can't read RotationOrder from {}", fileName);
                        return FALSE;
                }
                ...
                if (numRead != 3)
                {
                        ALOG_CRITICAL("can't read Scale from {}", fileName);
                        return FALSE;
                }
                setScale( scale );
                ...
                if (!isLOD())
                {
                        ...
                        if (numRead != 1)
                        {
                                ALOG_CRITICAL("can't read NumVertices from {}", fileName);
                                return FALSE;
                        }
						...
						for (U16 i = 0; i < numVertices; ++i)
						{
							...
							if (numRead != 3)
							{
									ALOG_CRITICAL("can't read Coordinates from {}", fileName);
									return FALSE;
							}
						}
						...
						for (U16 i = 0; i < numVertices; ++i)
						{
							...
							if (numRead != 3)
							{
									ALOG_CRITICAL(" can't read Normals from {}", fileName);
									return FALSE;
							}
						}
						...
						for (U16 i = 0; i < numVertices; ++i)
						{
							...
							if (numRead != 3)
							{
									ALOG_CRITICAL(" can't read Binormals from {}", fileName);
									return FALSE;
							}
						}
                        ...
                        {
                                ALOG_CRITICAL("can't read TexCoords from {}", fileName);
                                return FALSE;
                        }
                        ...
                        if (mHasDetailTexCoords)
                        {
                                ...
                                if (numRead != numVertices)
                                {
                                        ALOG_CRITICAL("can't read DetailTexCoords from {}", fileName);
                                        return FALSE;
                                }
                        }
                        ...
                        if (mHasWeights)
                        {
                                ...
                                if (numRead != numVertices)
                                {
                                        ALOG_CRITICAL("can't read Weights from {}", fileName);
                                        return FALSE;
                                }
                        }
                }
                ...
                if (numRead != 1)
                {
                        ALOG_CRITICAL("can't read NumFaces from {}", fileName);
                        return FALSE;
                }
                ...
                for (i = 0; i < numFaces; i++)
                {
                        ...
                        if (numRead != 3)
                        {
                                ALOG_CRITICAL("can't read Face[{}] from {}", i, fileName);
                                return FALSE;
                        }
                        if (mReferenceData)
                        {
...
//                      }
                ...
                ALOG_DEBUG("verts: {}, faces:{}, tris: {}",numVertices, numFaces, numTris);
                ...
                if (!isLOD())
                {
                        ...
                        if ( mHasWeights )
                        {
                                ...
                                if (numRead != 1)
                                {
                                        ALOG_CRITICAL("can't read NumSkinJoints from {}", fileName);
                                        return FALSE;
                                }
                        ...
                        for (i=0; i < numSkinJoints; i++)
                        {
                                ...
                                if (numRead != 1)
                                {
                                        ALOG_CRITICAL("can't read Skin[{}].Name from {}", i, fileName);
                                        return FALSE;
                                }
                        ...
                        if (fread(&numRemaps, sizeof(S32), 1, fp) == 1)
                        {
                                ...
                                for (S32 i = 0; i < numRemaps; i++)
                                {
                                        ...
                                        {
                                                ALOG_CRITICAL("can't read source vertex in vertex remap data");
                                                break;
                                        }
                                        if (fread(&remapDst, sizeof(S32), 1, fp) != 1)
                                        {
                                                ALOG_CRITICAL("can't read destination vertex in vertex remap data");
                                                break;
                                        }
        ...
        {
                ALOG_CRITICAL("invalid mesh file header: {}", fileName);
                status = FALSE;
        }
...
LLPolyMesh *LLPolyMesh::getMesh(const std::string &name, LLPolyMesh* reference_mesh)
{
        ...
        if (meshSharedData)
        {
//              ALOG_INFO("Polymesh {} found in global mesh table.", name);
...
//      ALOG_INFO("Polymesh {} added to global mesh table.", name);
...
void LLPolyMesh::dumpDiagInfo()
{
        ...
        ALOG_INFO("-----------------------------------------------------");
        ALOG_INFO("       Global PolyMesh Table (DEBUG only)");
        ALOG_INFO("   Verts    Faces  Mem(KB) Name");
        ALOG_INFO("-----------------------------------------------------");
