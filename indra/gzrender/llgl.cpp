...
bool LLGLManager::initVulkanInstance() {
	bool retVal = false;
...
#elif LL_DARWIN
	mInstanceExtensions.push_pack(VK_EXT_METAL_SURFACE_EXTENSION_NAME);
#endif
	...
	app_info.pNext = NULL;
	app_info.pApplicationName = "Alchemy Viewer";
	app_info.applicationVersion = 1;
	app_info.pEngineName = "SecondLife Gz Render Engine";
	...
	inst_info.enabledExtensionCount = static_cast<uint32_t>(mInstanceExtensions.size());
	inst_info.ppEnabledExtensionNames = mInstanceExtensions.data();
	inst_info.enabledLayerCount = 0;
	inst_info.ppEnabledLayerNames = NULL;

	if (vkCreateInstance(&inst_info, nullptr, &instance) != VK_SUCCESS) {
		LL_INFOS("RenderInit") << "Could not create a Vulkan instance." << LL_ENDL;
	}
	else {
		LL_INFOS("RenderInit") << "Created a Vulkan instance with extensions:" << LL_ENDL;
		for (auto ext : mInstanceExtensions) {
			LL_INFOS("RenderInit") << ext << LL_ENDL;
		}
...
bool LLGLManager::initVulkan() {
	...
	if (gpuCount > 0) {
		...
		VkResult vkerr = vkEnumeratePhysicalDevices(instance, &gpuCount, physicalDevices.data());

		U32 graphicsQueueIdx = 0;

		if (vkerr == VK_SUCCESS) {
			LL_INFOS("RenderInit") << "Found devices:" << LL_ENDL;
			// We likely want the GPU with the most amount of memory
			for (auto device : physicalDevices) {
				VkPhysicalDeviceProperties props;
				vkGetPhysicalDeviceProperties(device, &props);
				LL_INFOS("RenderInit") << props.deviceName << LL_ENDL;

				// Right now we only pick the first discrete GPU that supports a graphics queue.
				if (props.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) {
					...
					for (auto family : queueFamilies) {
						if (family.queueFlags & VK_QUEUE_GRAPHICS_BIT) {
							...
							mSelectedDeviceProperties = props;

							vkGetPhysicalDeviceFeatures(mSelectedDevice, &mSelectedDeviceFeatures);
							vkGetPhysicalDeviceMemoryProperties(mSelectedDevice, &mMemoryProperties);
							graphicsQueueIdx = i;
							break;
						}
						i++;
		...
		if (mSelectedDevice == VK_NULL_HANDLE) {
			// We shouldn't get here, but if we do- return.
			return false;
		}

		VkDeviceQueueCreateInfo queueCreateInfo{};
		queueCreateInfo.sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
		queueCreateInfo.queueFamilyIndex = graphicsQueueIdx;
		...
		createInfo.pEnabledFeatures = &mSelectedDeviceFeatures;
