...
void LLThreadLocalPointerBase::set( void* value )
{
	...
	if (result != APR_SUCCESS)
	{
		...
		ALOG_CRITICAL("Failed to set thread local data");
	}
}
...
void* LLThreadLocalPointerBase::get() const
{
	...
	{
		ll_apr_warn_status(result);
		ALOG_CRITICAL("Failed to get thread local data");
	}
	return ptr;
}
...
void LLThreadLocalPointerBase::initStorage( )
{
	...
	{
		ll_apr_warn_status(result);
		ALOG_CRITICAL("Failed to allocate thread local data");
	}
}
...
void LLThreadLocalPointerBase::destroyStorage()
{
	if (sInitialized)
	{
		if (mThreadKey)
		{
			...
			{
				ll_apr_warn_status(result);
				ALOG_CRITICAL("Failed to delete thread local data");
			}
		}
	}
}
